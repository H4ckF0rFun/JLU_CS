...
/*
	本代码是2021级数据结构课程设计 亚马逊棋的代码
	
	本代码中的MCTS实现并不正确, (概率论没学好,不知道正确的MCTS应该是啥样子的,也不会推导MCTS)
	至于参数,手动调参,大概是 king_move 越大,策略越偏向于占领空地。
	
	比较特别的一点是本代码中的board 的表示 是用的 uint64_t 而且所有参数
	的计算都是通过巧妙的位运算来进行(参考其他大佬的,实在是太强了)
	
	至于进一步的优化,本人数学太烂了,无从下手.
*/